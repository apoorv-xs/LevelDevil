...
async function pushBrainToFirestoreUI() {
  ...
  try {
    if (window.SALES_PLATFORM_AUTH?.getFirestore) {
      showNotification('☁️ Syncing knowledge nodes to Cloud Firestore...');
      const db = await window.SALES_PLATFORM_AUTH.getFirestore();
      await brain.pushToFirestore(db);
      showNotification('✅ All knowledge nodes saved to Cloud Firestore!');
      playSound('celebrate');
      updateBrainTelemetryView();
    } else {
      showNotification('Cloud Firestore auth adapter not initialized.', 'error');
    }
  } catch (err) {
    console.error("Firestore push error:", err);
    showNotification(`Firestore push failed: ${err.message}`, 'error');
  }
}
...
async function syncBrainFromFirestoreUI() {
  ...
  try {
    if (window.SALES_PLATFORM_AUTH?.getFirestore) {
      showNotification('🔄 Pulling knowledge nodes from Cloud Firestore...');
      const db = await window.SALES_PLATFORM_AUTH.getFirestore();
      await brain.syncWithFirestore(db);
      showNotification('✅ Knowledge graph synchronized from Cloud Firestore!');
      playSound('celebrate');
      renderBrainStudio();
    } else {
      showNotification('Cloud Firestore auth adapter not initialized.', 'error');
    ...
    showNotification(`Firestore sync failed: ${err.message}`, 'error');
  }
}
