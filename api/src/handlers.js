import { parseBody, validate, ok, checkRateLimit } from "./http.js";
import { guard } from "./auth.js";
import * as service from "./services.js";
import { qualify } from "./ai.js";
export const publicInquiry = (req) => { checkRateLimit(req, 60); const body = validate(parseBody(req), { name: { required: true, type: "string", max: 120 }, email: { required: true, type: "string", max: 254, format: "email" }, message: { required: true, type: "string", max: 5000 }, scope: { type: "string", max: 200 }, budget: { type: "string", max: 100 } }); return ok(service.createInquiry(body), 201); };
export const publicApplication = (req) => { checkRateLimit(req, 60); const body = validate(parseBody(req), { name: { required: true, type: "string", max: 120 }, email: { required: true, type: "string", max: 254, format: "email" }, message: { required: true, type: "string", max: 5000 }, resumeUrl: { type: "string", max: 2048, format: "url" } }); return ok(service.createApplication(body), 201); };
